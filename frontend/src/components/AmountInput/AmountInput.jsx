...
export default function AmountInput({ amount, setAmount, ethBalance }) {
  ...
  const adjustAmount = async (percentage) => {
    const newAmount = (ethBalance * percentage); // Use toFixed(4) to limit to 4 decimal places
    ...
    <div>
      ...
      <div className="relative mt-2 rounded-md shadow-sm">
        ...
        <input
          ...
          name="price"
          id="price"
          className="block w-full rounded-md border-0 py-1.5 pl-7 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 invalid:[&:not(:placeholder-shown):not(:focus)]:border-red-500"
          placeholder="0.00"
          value={amount}
